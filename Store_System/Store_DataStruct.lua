...
function NavData.Load(player)
    NavData.Cache = {}

    local Query = WorldDBQuery("SELECT * FROM store.store_categories")
    if (Query) then
        repeat
            local catId = Query:GetUInt32(KEYS.category.id - 1)

            -- Ocultar categoría 998 (Hermandad) si no es Guild Master
            if catId == 998 then
                if not player:IsInGuild() or player:GetRank() ~= 0 then
                    goto continue -- Salta esta categoría
                end
            end

            -- Insertar categoría en la caché
