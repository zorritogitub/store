...
local CONFIG = {
	...
	strings = {
		...
		insufficientFunds = "No tienes suficiente",
		-- Type is appended, ie. title, mount, pet etc.
		alreadyKnown = "Ya tienes esto",
		tooHighLevel = "Tu nivel es demasiado alto",
		mailBody = "¡Gracias por tu compra!",
		-- The service name is prefixed to this message
		successfulPurchase = "comprado exitosamente!"
	}
}
...
local function CanAttemptPurchase(player)
    local guid = player:GetGUIDLow()
    local now = os.clock()

    -- 1 segundo de cooldown anti spam
    if lastPurchaseAttempt[guid] and now - lastPurchaseAttempt[guid] < 1 then
        return false
    end

    lastPurchaseAttempt[guid] = now
    return true
end

function StoreHandler.Purchase(player, serviceId)
    -- Anti-spam / anti-doble click
    if not CanAttemptPurchase(player) then
        return
    end
...
function SHOP_UI.DeductCurrency(player, currencyId, amount)
    ...
    if(CURRENCY_TYPES[currencyType] == "GOLD") then
        if(player:GetCoinage() < amount * 10000) then
            ...
            player:ClearUnitState(0xFFFFFFFF) -- 🔧 limpia estados activos del jugador (previene intentos del cliente)
            return false
        end
    ...
    if(CURRENCY_TYPES[currencyType] == "ITEM_TOKEN") then
        if not(player:HasItem(currencyData, amount)) then
            player:SendAreaTriggerMessage("|cFFFF0000"..CONFIG.strings.insufficientFunds.." "..currencyName.."|r")
            player:PlayDirectSound(GetSoundEffect("notEnoughMoney", player:GetRace(), player:GetGender()), player)
            player:ClearUnitState(0xFFFFFFFF)
            return false
        end
...
function SHOP_UI.ItemHandler(player, data)
	...
	-- Send reward mail
	SendMail("Compra de: "..data[KEYS.service.name], CONFIG.strings.mailBody, player:GetGUIDLow(), CONFIG.mailSenderGUID, 62, 0, 0, 0, unpack(items))
	return true
end
